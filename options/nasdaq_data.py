...
class NasdaqOptions(object):
    ...
    def get_options_table(self):
        ...
        while loop < int(page_nb):
            ...
            url = 'http://www.nasdaq.com/symbol/' + self.ticker + '/option-chain?excode=' + self.market + self.money + '&expir=' + self.expi + '&dateindex=' + str(
                self.nearby) + '&page=' + str(loop + 1)

            # Query NASDAQ website
            try:
                print(url)

                response = requests.get(url,headers={"User-Agent": "Mozilla/5.0 (X11; CrOS x86_64 12871.102.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.141 Safari/537.36"})  # , timeout=0.1)
                #print("hi")
